...
if (!function_exists('get_first_parent category')) {
    function get_first_parent_category($SECTION_ID = null, $step = 15)
    {
        $resultArray = [];
        if (!empty($SECTION_ID) && $step > 0) {
            $res = CIBlockSection::GetByID((int)$SECTION_ID);
            if ($result = $res->GetNext()) {
                if (isset($result['IBLOCK_SECTION_ID'])) {
                    $resultArray = get_first_parent_category($result['IBLOCK_SECTION_ID'], $step - 1);
                }
                else
                    $resultArray = $result;
            }
...
if (!empty($arResult['ITEMS'])) {
    ...
    foreach ($arResult['ITEMS'] as &$item) {
        $categoryItem = get_first_parent_category($item['IBLOCK_SECTION_ID'], 15);
        $arResult['CATALOG_SECTIONS_ITEMS'][$categoryItem['ID']] = $categoryItem;
    }
