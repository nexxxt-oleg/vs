...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		showErrors: function(errors, scroll, showAll)
		{
			...
			{
				section = BX.findParent(errorNodes[k], {className: 'card-head-grey'});
		...
		checkNotifications: function()
		{
			...
			if (informer)
			{
				...
				{
					sections = this.orderBlockNode.querySelectorAll('.card-head-grey.bx-active');
		...
		{
			var sections = this.orderBlockNode.querySelectorAll('div.card-head-grey.bx-active'),
				i, errorNode;
		...
		{
			var firstSection = this.orderBlockNode.querySelector('.card-head-grey.bx-active');
		...
		locationsCompletion: function()
		{
			...
			for (i in this.locations)
			{
				...
				if (!this.firstLoad && this.options.propertyValidation)
				{
					if (inputStep)
					{
						...
						section = BX.findParent(locationNode, {className: 'card-head-grey'});
			...
			if (this.activeSectionId !== this.regionBlockNode.id)
				this.editFadeRegionContent(this.regionBlockNode.querySelector('.card-head-grey__content'));

			if (this.activeSectionId != this.propsBlockNode.id)
				this.editFadePropsContent(this.propsBlockNode.querySelector('.card-head-grey__content'));
		},
		...
		clickNextAction: function(event)
		{
			...
			{
				titleNode = section.next.querySelector('.card-head-grey__head');
				BX.bind(titleNode, 'click', BX.proxy(this.showByClick, this));
				editStep = section.next.querySelector('.card-head-grey__edit');
				if (editStep)
					editStep.style.display = '';

				allSections = this.orderBlockNode.querySelectorAll('.card-head-grey.bx-active');
		...
		getNextSection: function(actionSection, skippedSection)
		{
			...
			var allSections = this.orderBlockNode.querySelectorAll('.card-head-grey.bx-active'),
				nextSection, i;
		...
		markSectionAsCompleted: function(section)
		{
			...
			)
			{
				...
				titleNode = section.querySelector('.card-head-grey__head');
				BX.bind(titleNode, 'click', BX.proxy(this.showByClick, this));
			}
		...
		{
			var sections = this.orderBlockNode.querySelectorAll('.card-head-grey.bx-active'),
				firstSection = sections[0],
				lastSection = sections[sections.length - 1],
				currentSection = BX.findParent(node, {className: "card-head-grey"}),
				isLastNode = false,
				buttons = [];
				...
				buttons.push(
					BX.create('A', {
						props: {
							...
							className: 'btn btn_arrow_left po-cards-btm__prev-btn'
						},
						children: [
							BX.create('IMG', {
								props: {
									src: '/bitrix/templates/stroyka_reborn/img/ui-icon/arrow-left-sm.svg',
									className: 'img-svg'
								}
							}),
							BX.create('SPAN', {
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {href: 'javascript:void(0)', className: 'btn btn_color_ac po-cards-btm__next-btn'},
			...
			node.appendChild(
				BX.create('DIV', {
					props: {className: 'card-head-grey__bottom po-cards-btm'},
					children: buttons
				})
			);
		},
		...
		{
			return BX.create('DIV', {props: {className: 'card-head-grey__content'}});
		},
		...
		shouldBeSectionVisible: function(sections, currentPosition)
		{
			...
			for (; currentPosition < sections.length; currentPosition++)
			{
				...
				if (!this.firstLoad)
				{
					editStepNode = sections[currentPosition].querySelector('.card-head-grey__edit');
		...
		changeVisibleSection: function(section, state)
		{
			...
			{
				content = section.querySelector('.card-head-grey__content');
				if (content)
					content.style.display = state ? '' : 'none';
			}

			editStep = section.querySelector('.card-head-grey__edit');
			if (editStep)
		...
		editOrder: function()
		{
			...
			var sections = this.orderBlockNode.querySelectorAll('.card-head-grey.bx-active'), i;
			for (i in sections)
			{
				if (sections.hasOwnProperty(i))
				{
		...
		editSection: function(section)
		{
			...
			{
				BX.bind(titleNode, 'click', BX.proxy(this.showByClick, this));
				editButton = titleNode.querySelector('.card-head-grey__edit');
		...
		editAuthBlock: function()
		{
			...
			var authContent = this.authBlockNode.querySelector('.card-head-grey__content'),
			...
			if (this.result.OK_MESSAGE && this.result.OK_MESSAGE.length)
			{
				...
				BX.prepend(okMessageNode, this.authBlockNode.querySelector('.card-head-grey__content'));
			}
		},
		...
		toggleAuthForm: function(event)
		{
			...
			var target = event.target || event.srcElement,
				section = BX.findParent(target, {className: 'card-head-grey'}),
				container = BX.findParent(target, {className: 'card-head-grey__content'}),
			...
			section.appendChild(
				BX.create('DIV', {
					props: {className: 'card-head-grey__head'},
					children: [
						BX.create('h4', {
							props: {className: 'card-head-grey__title'},
		...
		editActiveBasketBlock: function(activeNodeMode) {
			...
			if (this.initialized.basket) {
				...
				basketContent = node.querySelector('.card-head-grey__content');
				basketTable = BX.create('DIV', {props: {className: 'card-head-grey__items'}});
		...
		editFadeBasketBlock: function(activeNodeMode)
		{
			...
			if (this.initialized.basket)
			{
				this.basketHiddenBlockNode.appendChild(node.querySelector('.card-head-grey__content'));
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++)
			{
				...
				mainColumns.push(this.createBasketItemColumn(currentColumn, item, active));

				++basketColumnIndex;
				if (basketColumnIndex == 4 && this.result.GRID.HEADERS[i + 1])
				{
					mainColumns.push(BX.create('DIV', {props: {className: 'bx-soa-item-nth-4p1'}}));
					basketColumnIndex = 0;
				}
			}
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'card-head-grey__item' + (index == 0 ? ' card-head-grey__item_first' : '')},
					children: [
						BX.create('DIV', {
							props: {className: 'card-product-2'},
		...
		createBasketItemImg: function(data)
		{
			...
			if (data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length) {
				logoNode = BX.create('A', { props: { 
					className: 'card-product-2__img',
					href: data.DETAIL_PAGE_URL
				}});
			} else {
				logoNode = BX.create('DIV', { props: { className: 'card-product-2__img' }});
			}
		...
		createBasketItemContent: function(data)
		{
			...
			if (data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length)
				titleHtml = '<a href="' + data.DETAIL_PAGE_URL + '" class="card-product-2__link">' + titleHtml + '</a>';

			return BX.create('DIV', {
				props: {
					className: 'card-product-2__name'
				},
				html: titleHtml
			});
		},
		...
		createBasketItemColumn: function(column, allData, active) {
			...
			} else if (column.id === 'SUM') { // Общая стоимость
				parentClass = 'card-product-2__price-3';
				textNode.setAttribute('class', 'b-price');
				textNode.appendChild(BX.create('SPAN', {props: {className: 'b-price__number'}, html: data.SUM}));
			...
			} else if( column.id === 'QUANTITY') { // Количество товаров
				parentClass = 'card-product-2__price-2';
				textNode.setAttribute('class', 'b-amount');
			...
			return BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						props: {className: 'card-sm-title'},
						children: [
							BX.create('P', {text: column.name}),
							textNode
						]
					})
				]
		...
		editCoupons: function(basketItemsNode)
		{
			var couponsList = this.getCouponsList(true),
				couponsLabelText = this.getCouponsLabelText(true),
				couponsBlock = BX.create('DIV', {
					props: {className: 'card-coupon-order'},
					children: [
						BX.create('DIV', {
							props: {className: 'card-coupon-order__input form-field'},
							children: [
								BX.create('INPUT', {
									props: {
										className: 'form-field__input',
										type: 'text',
										placeholder: couponsLabelText
									},
									events: {
						...
						BX.create('SPAN', {props: {className: 'card-coupon-order__item'}, children: couponsList})
					]
				});

			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'card-head-grey__coupon'},
					children: [
						couponsBlock
					]
				})
		...
		getCouponsLabelText: function(active) {
			return active ? this.params.MESS_USE_COUPON : this.params.MESS_COUPON;
		},
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			...
			{
				regionContent = node.querySelector('.card-head-grey__content');
				if (!regionContent)
				{
		...
		{
			var regionContent = this.regionBlockNode.querySelector('.card-head-grey__content'), newContent;
		...
		editActivePaySystemBlock: function(activeNodeMode)
		{
			...
			{
				paySystemContent = node.querySelector('.card-head-grey__content');
				if (!paySystemContent)
				{
		...
		{
			var paySystemContent = this.paySystemBlockNode.querySelector('.card-head-grey__content'), newContent;
		...
		editFadePaySystemContent: function(node)
		{
			...
			if (this.isSelectedInnerPayment())
			{
				...
				addedHtml += '<div class="card-head-grey__item">';
				addedHtml += '<div class="card-order-radio">';
				addedHtml += '<label for="payment-0" class="card-order-radio__label">';
				addedHtml += '<div class="card-order-radio__img">';
				addedHtml += '<img src="' + imgSrc + '" style="height:18px;">';
				addedHtml += '</div>';
				addedHtml += '<div class="card-order-radio__name b-content">';
				addedHtml += '<p>' + this.result.INNER_PAY_SYSTEM.NAME + '</p>';
				addedHtml += '</div>';
				addedHtml += '<div class="card-order-radio__desc b-content">';
				addedHtml += '<p></p>';
				addedHtml += '</div>';
				addedHtml += '</label>';
				addedHtml += '</div>';
			...
			if (selectedPaySystem && selectedPaySystem.NAME)
			{
				...
				addedHtml += '<div class="card-head-grey__item">';
				addedHtml += '<div class="card-order-radio card-order-radio_checked">';
				addedHtml += '<div for="payment-0" class="card-order-radio__label">';
				addedHtml += '<div class="card-order-radio__img">';
				addedHtml += '<img src="' + imgSrc + '" style="height:18px;">';
				addedHtml += '</div>';
				addedHtml += '<div class="card-order-radio__name b-content">';
				addedHtml += '<p>' + BX.util.htmlspecialchars(selectedPaySystem.NAME) + '</p>';
				addedHtml += '</div>';
				addedHtml += '<div class="card-order-radio__desc b-content">';
				addedHtml += '<p></p>';
				addedHtml += '</div>';
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			{
				deliveryContent = node.querySelector('.card-head-grey__content');
				if (!deliveryContent)
				{
		...
		{
			var deliveryContent = this.deliveryBlockNode.querySelector('.card-head-grey__content'), newContent;
		...
		editFadeDeliveryContent: function(node)
		{
			...
			if (selectedDelivery && selectedDelivery.NAME)
			{
				...
				arNodes = BX.create('DIV', {
					props: {
						className: 'card-order-radio card-order-radio_checked'
					},
					children: [
						BX.create('LABEL', {
							props: {
								className: 'card-order-radio__label',
								htmlFor: 'delivery'
							},
							children: [
								BX.create('DIV', {
									props: {
										className: 'card-order-radio__img'
									},
									children: [
										BX.create('IMG', {
											props: {src: imgSrc, alt: ''}, style: {height: '18px'}
										})
									]
								}),
								BX.create('DIV', {
									props: {
										className: 'card-order-radio__name b-content'
									},
									children: [
										BX.create('P', {text: name})
									]
								}),
								BX.create('DIV', {
									props: {
										className: 'card-order-radio__desc b-content'
									},
									children: [
										BX.create('P', {})
									]
								...
								BX.create('DIV', {
									props: {
										className: 'card-order-radio__price'
									},
									children: [
										BX.create('DIV', {
											props: {
												className: 'card-sm-title'
											},
											children: [
												BX.create('P', {
													text: 'Стоимость'
												...
												BX.create('P', {
													...
													children: [
														BX.create('SPAN', {
															text: this.getDeliveryPriceNodes(selectedDelivery)
														})
													]
												})
											]
				...
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'card-head-grey__item'},
						children: [arNodes]
					})
				);
			}
		...
		activatePickUp: function(deliveryName)
		{
			...
			this.pickUpBlockNode.querySelector('h4.card-head-grey__title').innerHTML = BX.util.htmlspecialchars(deliveryName);
		...
		editActivePickUpBlock: function(activeNodeMode)
		{
			...
			{
				pickUpContent = node.querySelector('.card-head-grey__content');
				if (!pickUpContent)
				{
		...
		{
			var pickUpContent = this.pickUpBlockNode.querySelector('.card-head-grey__content'), newContent;
		...
		editActivePropsBlock: function(activeNodeMode)
		{
			...
			{
				propsContent = node.querySelector('.card-head-grey__content');
				if (!propsContent)
				{
		...
		{
			var propsContent = this.propsBlockNode.querySelector('.card-head-grey__content'), newContent;
		...
		basketBlockScrollCheck: function()
		{
			...
			for (i = 0; i < scrollableNodes.length; i++)
			{
				...
				tableNode = parentNode.querySelector('div.card-head-grey__items');
